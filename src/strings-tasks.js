...
function getFirstChar(value) {
  const getFirstCharset = value.charAt(0);
  return getFirstCharset;
}
...
function removeLeadingWhitespaces(value) {
  const getTrimmed = value.trimStart();
  return getTrimmed;
}
...
function removeTrailingWhitespaces(value) {
  const getTrimmed = value.trimEnd;
  return getTrimmed;
}
...
function repeatString(str, times) {
  const repeatFun = str.repeat(times);
  return repeatFun;
}
